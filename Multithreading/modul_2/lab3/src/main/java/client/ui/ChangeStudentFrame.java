...
public class ChangeStudentFrame extends ChangeObjectFrame {
    ...
    public ChangeStudentFrame(Client client, Point parentLocation) {
        super("Change student data", client, parentLocation);
    ...
    protected void submitChanges() {
        ...
        try {
            ...
            if ((newGroupName != null) && !newGroupName.equals("") && !newGroupName.equals(groupName)) {
                groupId = client.getGroupIdFromDatabase(newGroupName);
            } else {
                groupId = client.getGroupIdFromDatabase(groupName);
            }
            if (groupId == -1) {
                throw new Exception("Cannot resolve group name");
            }
            Student student = Utilities.createStudent(studentId, firstName, lastName, address, phoneNumber, groupId);
            client.updateStudentInDatabase(student);
    ...
    private void responseAction(ActionEvent e) {
        if (e.getSource() == groupComboBox) {
            Group group = client.getGroupFromDatabase((String) groupComboBox.getSelectedItem());
